...
function thisLineItem($patient_id, $encounter_id, $rowcat, $description, $provider, $transdate, $entdate, $qty, $amount, $irnumber = '')
{
    global $product, $category, $producttotal, $productqty, $cattotal, $catqty, $grandtotal, $grandqty;
    global $productleft, $catleft, $providerleft;
  ...
  <td align="right">
    ...
    if (!empty($_POST['form_details'])) {
        if ($_POST['form_csvexport']) {
            echo csvEscape(display_desc($category)) . ',';
            echo csvEscape(display_desc($product)) . ',';
             echo csvEscape(display_desc($provider)) . ',';
            echo csvEscape(oeFormatShortDate(display_desc($transdate))) . ',';
            echo csvEscape(oeFormatShortDate(display_desc($entdate))) . ',';
  ...
  <td class="detail">
    ...
    <td class="detail">
            <?php echo text(display_desc($providerleft));
            $providerleft = " "; ?>
  </td>
  <td>
         ...
         <td>
             <?php echo text(oeFormatShortDate($entdate)); ?>
         </td>
...
if (!empty($_POST['form_refresh']) || !empty($_POST['form_csvexport'])) {
    ...
    "b.code_text, fe.date, fe.entered_date, fe.facility_id, fe.provider_id, fe.invoice_refno, lo.title " .
    "FROM billing AS b " .
